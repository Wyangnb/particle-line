class particleLine {
    constructor(data) {
        ...
        this.Point = class Point{
            ...
            move(x, y) {
                ...
                if (this.x > that.canvas.width || this.x < 0) this.sx = -this.sx;
                if (this.y > that.canvas.height || this.y < 0) this.sy = -this.sy;
            }
            ...
            drawLine (ctx, p){
                let dx = this.x -p.x;
                let dy = this.y -p.y;
                let d = that.isBg? dx * dx + dy * dy: Math.sqrt(dx * dx + dy * dy);
    ...
    play() {
        ...
        this.mons.lizi = new this.Point(Math.random() * this.canvas.width, Math.random() * this.canvas.height, 20000, 'touch');
